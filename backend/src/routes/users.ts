...
router.post("/", requireAuth, requireRole(["admin"]), async (req: Request, res: Response) => {
  try {
    ...
    await createAuditLog({ action: "user_created", actorUserId: (((req.user as any).userId as string)), targetType: "user", targetId: id, details: `Created user ${email}` });
...
router.put("/:id", requireAuth, requireRole(["admin"]), async (req: Request, res: Response) => {
  try {
    ...
    await createAuditLog({ action: "user_updated", actorUserId: (((req.user as any).userId as string)), targetType: "user", targetId: req.params.id, details: "User updated" });
...
router.patch("/:id/team", requireAuth, requireRole(["admin"]), async (req: Request, res: Response) => {
  ...
  try {
    ...
    await createAuditLog({ action: "user_team_updated", actorUserId: (((req.user as any).userId as string)), targetType: "user", targetId: req.params.id, details: "User team updated" });
